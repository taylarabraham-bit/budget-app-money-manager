...
The app is offline-first: every phone or PC keeps its own copy of the household's data and mirrors changes to a Supabase server whenever it can reach it. There are no accounts and there is no hosted service behind the app. **Each household brings its own server.** Nothing in this repository points at any particular one: a fresh clone, and an APK built from it, connects to nothing until you give it your own server's URL and anon key, in the app's Settings or in a gitignored `app/.env.local`. The two files in [`migrations/`](migrations/) are the only server-side code, and they are the same for everyone and for both routes below.

There are two ways to get a server.

## Hosted Supabase project (fastest)

A free project on supabase.com. Nothing to install and nothing at home has to stay on. The trade-off: the project is reachable from the whole internet, so the URL plus the key are the only lock (see [What must stay true](#what-must-stay-true-audit-sec-6)).

1. Create a free account at https://supabase.com and create a new project. Any name; choose a strong database password (the app never needs it).
2. Open the **SQL Editor** and run both migration files, in order, one after the other: paste [`migrations/20260823_sync_records.sql`](migrations/20260823_sync_records.sql) and run it, then [`migrations/20260902_clock_clamp.sql`](migrations/20260902_clock_clamp.sql) and run it. Both are safe to run again later.
3. Open **Project Settings > API** and copy the **Project URL** and the **anon public** key (newer dashboards may list it as the publishable key; either works).
4. Give the app those two values on each device: avatar (top right) > **Settings** > **Sync between devices** > paste the Project URL into **Server URL** and the key into **Anon key** > **Connect**. See [On each device](#on-each-device) for what happens on the first connection.

Free projects are paused after about a week without activity. The app then shows **Server not reachable**, keeps working offline and queues changes; restore the project from the Supabase dashboard and the queue goes through. If the key leaks, generate a new JWT secret in **Project Settings > API**: the old key stops working at once, and the new one has to be pasted into each device's **Settings > Sync between devices**.

## Self-hosted at home over Tailscale (most private)

Supabase runs on a PC or laptop at home, reachable only over the household's private [Tailscale](https://tailscale.com) network. The network is the lock. The rest of this document is that route.
...
- **Baked into a build**: copy [`app/.env.example`](../app/.env.example) to `app/.env.local` and fill it in. `npm run dev` and every build made from that clone then connect automatically, and an APK built there carries the values as its default. `.env.local` is gitignored; never commit the real key. A CI or GitHub Pages build has no `.env.local` and so ships with no default server.
...
- **On a hosted project there is no tailnet, so the key is the only lock.** The URL plus the anon key give full read, write and hard-delete on the household to anyone who holds them, from anywhere. Keep both private, and rotate the JWT secret from the dashboard the moment either leaks.
...
1. For the self-hosted route, install and sign in to Tailscale on the phone or PC so it can reach the server. A hosted project needs only an internet connection.
2. In the app: avatar (top right) > **Settings** > **Sync between devices** > paste the URL and the anon key > **Connect**. A device still in the setup wizard can instead choose **Join your partner's household** and use **Connect and join**.
3. The first device to connect uploads its household. The second device, if it is still showing the sample household, joins automatically and asks which of you it is. If both already hold real data you are asked which side wins: **Use the server's data** or **Upload this device's data** in Settings, or **Join <the household's name>** or **Keep this device's data** in the setup wizard.
...
- Realtime (instant updates between devices) is on by default in the self-hosted stack and on hosted projects. If it is disabled the app still syncs every 30 seconds and whenever it comes to the foreground.
